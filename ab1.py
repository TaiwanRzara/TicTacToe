...
class TicTacToe:
    ...
    def evaluate(self)->int:
        score:int=0
        Winner=self.Winner()
        if(Winner>0 ): return 100 if Winner==1 else -100
        for i in range(2):
            for j in range(3):
...
def AB(G:TicTacToe,depth:int,a:int,b:int,isMaxPlayer:bool):
    if(G.Put==9):
        ...
        if(w==1 and isMaxPlayer):
            ...
            return [100,-1]
    if(depth==0): return [G.evaluate(),-1]
    if(isMaxPlayer==True):
        for x in range(3):
            for y in range(3):
                if(G.Broad[x][y]!=0): continue
                New=copy.deepcopy(G)
                New.put(x,y,2)
